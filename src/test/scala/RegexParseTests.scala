class RegexParseTests extends munit.FunSuite {
  ...
  test ("regex preprocess 3") {
    val str = "a|b|c"
    val expected = List(Left('a'), Right('|'), Left('b'), Right('|'), Left('c'))
    assertEquals(Regex.preprocess(str.toList), expected)
  }

  test ("regex preprocess 6") {
    val str = "abc"
    val expected = List(Left('a'), Right('.'), Left('b'), Right('.'), Left('c'))
    assertEquals(Regex.preprocess(str.toList), expected)
  }
  test ("regex preprocess 7") {
    val str = "(a|b)c"
    val expected = List(Right('('), Left('a'), Right('|'), Left('b'), Right(')'), Right('.'), Left('c'))
    assertEquals(Regex.preprocess(str.toList), expected)
  }

  test ("regex preprocess 8") {
    val str = "c(a|b)"
    val expected = List(Left('c'), Right('.'), Right('('), Left('a'), Right('|'), Left('b'), Right(')'))
    assertEquals(Regex.preprocess(str.toList), expected)
  }

  test ("regex preprocess 9") {
    val str = "ab*cd*"
    val expected = List(Left('a'), Right('.') ,Left('b'), Right('*'), Right('.') ,Left('c'), Right('.'), Left('d'), Right('*'))
    assertEquals(Regex.preprocess(str.toList), expected)
  }
  ...
  test ("infix to ast") {
    val str = "a*"
    val expected = "STAR a"
    assertEquals(Regex.toPrenex(str), expected)
  }
  test ("infix to ast 2") {
    val str = "a|b"
    val expected = "UNION a b"
    assertEquals(Regex.toPrenex(str), expected)
  }
  test ("infix to ast 3") {
    val str = "a|b|c"
    val expected = "UNION UNION a b c"
    assertEquals(Regex.toPrenex(str), expected)
  }
